...
.DESCRIPTION
    This script aims to remediate all possible OS baseline misconfigurations for Ubuntu 22.04 based Virtual machines.
.EXAMPLE
    Command to execute : bash CIS_Ubuntu_Hardening_Benchmark.sh
.INPUTS
...
echo "
*******************************************************
WARNING: OAF Authorized access only.
...
apt-get install rsyslog -y || apt-get install syslog-ng -y
...
# 5.4.1 Ensure system accounts are non-login
echo
echo -e "${RED}5.4.1${NC} Ensure system accounts are non-login"
...
echo
echo -e "${RED}5.4.2${NC} Ensure default group for the root account is GID 0"
usermod -g 0 root
policystatus=$?
if [[ "$policystatus" -eq 0 ]]; then
...
# 5.4.3 Ensure default user umask is 027 or more restrictive
echo
echo -e "${RED}5.4.3${NC} Ensure default user umask is 027 or more restrictive"
